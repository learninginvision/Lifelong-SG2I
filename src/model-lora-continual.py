...
class CustomDiffusion(LatentDiffusion):
    def __init__(self,
        ...
        def new_forward(self, x, context=None, mask=None,task_id=-1,mask_value=None,gate_c=None):
            ...
            if context is not None and mask_value is not None and x.shape[1]>=0:
                full_shape=int(x.shape[1]**(1/2))
                out = 0
                resized_mask_tensor_list = []
                batch_size = context.shape[0]//2
                q = rearrange(q, 'b n (h d) -> (b h) n d', h=h)   
                for i in range(len(mask_value[1])):  
                    region_prompt = torch.cat((context[0].unsqueeze(0).repeat(batch_size,1,1),mask_value[1][i]['prompt'].unsqueeze(0).repeat(batch_size,1,1)), dim=0)
                    if i != len(mask_value[1])-1:
                        if i ==0:
                            k = self.to_k(region_prompt,gate_c[i].unsqueeze(0), task_id=task_id)
                            v = self.to_v(region_prompt,gate_c[i].unsqueeze(0), task_id=task_id)
                        else: 
                            k = self.to_k(region_prompt, task_id=-1)
                            v = self.to_v(region_prompt, task_id=-1)
                        resized_mask_tensor = torch.nn.functional.max_pool2d(mask_value[1][i]['mask'].unsqueeze(0), kernel_size=int(64/full_shape), stride=int(64/full_shape))  #mask_value[i]['mask']  (batch_size 64 64)
                ...
                full_shape=int(x.shape[1]**(1/2))
                ##########
                if mask_value is not None and len(mask_value)>1 and not crossattn and x.shape[1]>=0 and mask_value[1][-1]['time_step']>750:
                    # print(mask_value[1][-1]['time_step'])
                    resized_mask_tensor_list = []
                    for i in range(len(mask_value[1])):
                        resized_mask_tensor = torch.nn.functional.max_pool2d(mask_value[1][i]['mask'].unsqueeze(0), kernel_size=int(64/full_shape), stride=int(64/full_shape)).view(-1)
                        resized_mask_tensor_list.append(resized_mask_tensor)
                    tokens_mask = torch.stack(resized_mask_tensor_list,dim=1)  
                    attention_mask = torch.matmul(tokens_mask, tokens_mask.T) > 0  # (N, N)
        ...
        def new_forward_lora(self, x, context=None, mask=None,task_id=-1):
            h = self.heads
            crossattn = False
            q = self.to_q(x,task_id=task_id)
            if context is not None:
                crossattn = True
                context = default(context, x)
                k = self.to_k(context,task_id=task_id)
                ...
                k = self.to_k(x,task_id=task_id)
                v = self.to_v(x,task_id=task_id)
            if crossattn:
                modifier = torch.ones_like(k)
                modifier[:, :1, :] = modifier[:, :1, :]*0.
                k = modifier*k + (1-modifier)*k.detach()
    ...
    def configure_optimizers(self):
        ...
        print("lr_route",lr_route)
        print("lr",lr_text_embedding)
        for x in self.model.diffusion_model.named_parameters():
           x[1].requires_grad = False
        params = []
        params_lora=[]
        ...
        if self.freeze_model == 'attn-lora':
            ...
            if self.lora_type=="loramoeonegate" or self.lora_type=="continuallora"or self.lora_type=="loraEC" or self.lora_type=="loraEC2" or self.lora_type=="MultiLora" or self.lora_type=="MoeDel":
                for x in self.model.diffusion_model.named_parameters():
                    if 'transformer_blocks' in x[0]:
    ...
    def get_zero_lora(self,model,task_id,input,device):
        output = 0.0
        for layer in model.children():
            if type(layer)==ContinualLoraInjectedLinear or type(layer)==ContinualLoraMoeOneGateInjectedLinear or type(layer)==ContinualSingleLora or type(layer)==ContinualLoraEC or type(layer)==ContinualLoraEC2 or type(layer)==ContinualMultiLora or type(layer)==ContinualMoewithDel:
    ...
    def get_loss_Hadamard(self,model,task_id, device):
        ...
        for layer in model.children():
            if type(layer)==ContinualLoraInjectedLinear or type(layer)==ContinualSingleLora:
    ...
    def p_losses(self, x_start, cond, t, mask=None,concept_mask=None, task_id=-1, noise=None):
        ...
        if self.loss_ortho == "FA" or self.loss_ortho == "FAO" or self.loss_ortho == "FAG" or self.loss_ortho == "FAGOF" or self.loss_ortho=="FACG":
