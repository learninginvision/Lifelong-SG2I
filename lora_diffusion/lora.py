...
# y=net(x,task_id=1)
class ContinualSingleLora(nn.Module):
    def __init__(
        self, in_features, out_features,bias=False,num_tasks=5,pool_size=5,r=8, dropout_p=0, scale=1
    ):
        super().__init__()
...
# y=net(x,task_id=2)
class ContinualLoraEC(nn.Module):
    def __init__(
        self, in_features, out_features,bias=False,num_tasks=5,pool_size=5,r=8, dropout_p=0,Moe_TopK=2, scale=1
    ):
        super().__init__()
    ...
    def loss_ortho(self, task_id=None,loss_type="FA",device=None):
        ...
        if  loss_type=="FAG":
            route_all = torch.cat([self.lora_route[:task_id-1].detach(), self.lora_route[task_id-1].unsqueeze(0)], dim=0).permute(0, 2, 1).reshape(-1, self.in_features)
            QQT_route = torch.mm(route_all, route_all.t())
            #matrix_lora = torch.ones(((task_id+1) * self.r, (task_id+1) * self.r),device=device)
            matrix_route = torch.ones((task_id * self.pool_size, task_id * self.pool_size),device=device)
            #for i in range(task_id+1):
            #matrix_lora[i * self.r: (i + 1) * self.r, i * self.r: (i + 1) * self.r] = torch.zeros(self.r, self.r)
            for i in range(task_id):
            ...
            #w0=self.linear.weight.unsqueeze(0).repeat(self.delta_w.shape[0], 1, 1)
            #w=w0+self.delta_w.transpose(-1,-2)
            #loss_lora=torch.norm(torch.einsum('ikl,ilm->ikm', w,w.transpose(-1, -2))-torch.einsum('ikl,ilm->ikm',w0,w0.transpose(-1, -2)),p='fro')
            if task_id>1:
            ...
            w0=self.linear.weight.unsqueeze(0).repeat(self.delta_w.shape[0], 1, 1)
            w=w0+self.delta_w.transpose(-1,-2)
            loss_w=torch.norm(torch.einsum('ikl,ilm->ikm', w,w.transpose(-1, -2))-torch.einsum('ikl,ilm->ikm',w0,w0.transpose(-1, -2)),p='fro')
    ...
    def del_lora(self, input,task_id=-1):
        ...
        if torch.mean(lora_omegas,dim=(0,1))[task_id-1]<0.4:
            print(torch.mean(lora_omegas,dim=(0,1))[task_id-1])
            with torch.no_grad():
                self.lora_up[task_id-1].zero_()  # 直接将 self.lora_up[task_id-1] 置为零
                self.lora_down[task_id-1].zero_()  # 直接将 self.lora_down[task_id-1] 置为零
    ...
    def forward(self, input,input2=None,task_id=-1):
        ...
        if task_id<=self.num_tasks:    
            lora_route_all=torch.cat([self.lora_route[task_id-1,:,:task_id],self.lora_route[task_id-1,:,task_id:].detach()],dim=1).unsqueeze(0)
            #lora_route_current = torch.cat((self.lora_route[task_id-1,:,:task_id], self.lora_route[task_id-1,:,task_id:].detach()), dim=-1).unsqueeze(0)
            ...
            #lora_route_all=torch.cat([self.lora_route[0,:,0].unsqueeze(-1).detach(),self.lora_route[0,:,1:2],self.lora_route[0,:,2:].detach()],dim=1).unsqueeze(0)
            lora_route_all=torch.sum(self.lora_route,dim=0).unsqueeze(0)
        task_id = min(task_id, self.num_tasks)
        if self.training:
            input_mean=torch.mean(input,dim=1).unsqueeze(1)
            ...
            lora_omegas=nn.functional.softmax(torch.matmul(input_mean,lora_route_all[:,:,:task_id]),2)
            lora_omegas_mean=torch.mean(lora_omegas,dim=(0,1))[:task_id-1]
            k=min(task_id-1,self.Moe_TopK-1) #top(k+1)
            ...
            #         lora_down_up = (self.lora_down[j] @ self.lora_up[j]).detach()
            #     scaled_transformed = self.scale * self.dropout(self.selector(input@lora_down_up))
            #     omega_scaled_transformed = torch.mean(lora_omegas[:, :, j],dim=1).view(-1,1,1)* scaled_transformed 
            #     delta_w_all+=omega_scaled_transformed
            for j in I_list:
                if j == task_id-1:
                    if self.requires_grad:    
                        lora_down_up = (self.lora_down[j] @ self.lora_up[j])
                         ...
                         with torch.no_grad():
                            self.lora_up[task_id-1].zero_()  # 直接将 self.lora_up[task_id-1] 置为零
                            self.lora_down[task_id-1].zero_()
                            lora_down_up = (self.lora_down[j] @ self.lora_up[j]).detach()
                            print(lora_down_up[0,0])
                else:
                    lora_down_up = (self.lora_down[j] @ self.lora_up[j]).detach()
                ...
                #if self.is_coft:
                    #with torch.no_grad():
                        #delta_w_all.copy_(project(delta_w_all, eps=1e-3))
            ...
            print(torch.mean(lora_omegas,dim=(0,1)))
            for j in I_list:
                #if torch.mean(lora_omegas[:, :, j],dim=(0,1))>=0.1:
                    lora_down_up = (self.lora_down[j] @ self.lora_up[j]).detach()
                    scaled_transformed = self.scale * self.dropout(self.selector(input@lora_down_up))
                    omega_scaled_transformed = torch.mean(lora_omegas[:, :, j],dim=1).view(-1,1,1)* scaled_transformed 
                    delta_w_all+=omega_scaled_transformed
...
x=torch.rand(2,77,768)
x2=torch.rand(1,77,768)
net=ContinualLoraEC(768,320,num_tasks=5,pool_size=5,r=16,dropout_p=0,scale=1)
y=net(x,x2,task_id=2)
class ContinualLoraEC2(nn.Module):
    def __init__(
    ...
    def loss_ortho(self, task_id=None,loss_type="FA",device=None):
        ...
        if  loss_type=="FAG":
            ...
            #w=w0+self.delta_w.transpose(-1,-2)
            #loss_w=torch.norm(torch.einsum('ikl,ilm->ikm', w,w.transpose(-1, -2))-torch.einsum('ikl,ilm->ikm',w0,w0.transpose(-1, -2)),p='fro')
            #loss_w=torch.norm(torch.matmul(w,w.T)-torch.matmul(w0,w0.T),p='fro')
            loss=loss_w+loss_gate+loss_c
        elif loss_type=="FAO": 
                loss = torch.norm(torch.mul(QQT_lora,1-torch.eye(QQT_lora.shape[0],device=device)), p='fro')
        elif loss_type=="FA": 
    ...
    def forward(self, input,input2=None,task_id=-1,scale=1):
            ...
            if task_id >= 1:
                if task_id<=self.num_tasks:
                    #lora_route_all=torch.cat([self.lora_route[task_id-1,:,:task_id],self.lora_route[task_id-1,:,task_id:].detach()],dim=1).unsqueeze(0)
                    #lora_route_current=torch.cat([self.lora_route[task_id-1,:,:task_id+1],self.lora_route[task_id-1,:,task_id+1:].detach()],dim=-1).unsqueeze(0)
                    lora_route_current = torch.cat((self.lora_route[task_id-1,:,0:1].detach(), self.lora_route[task_id-1,:,1:task_id+1], self.lora_route[task_id-1,:,task_id+1:].detach()), dim=-1).unsqueeze(0)
                    lora_route_last=torch.sum(self.lora_route[:task_id-1].detach(),dim=0).unsqueeze(0)
                    lora_route_all=lora_route_current+lora_route_last
                    # lora_route_all=lora_route_current
                ...
                # print(torch.mean(lora_omegas,dim=(0,1)))
                    ...
                    I_list = I.tolist()
                #     # #########################
                #     selected_gate_id = ~((gate_value<self.thres1)*(I==task_id))
...
class ContinualMoewithDel(nn.Module):
    def __init__(
        self, in_features, out_features,bias=False,num_tasks=5,pool_size=5,r=8,Moe_TopK=21, dropout_p=0, scale=1
    ):
        super().__init__()
        ...
        self.num_tasks=num_tasks
        self.dropout = nn.Dropout(dropout_p)
        self.lora_down = nn.ParameterDict({
            str(i): nn.Parameter(torch.zeros((in_features, r)), requires_grad=True) 
            for i in range(self.pool_size)
        })
        self.lora_up = nn.ParameterDict({
            str(i): nn.Parameter(torch.zeros((r,out_features)), requires_grad=True) 
            for i in range(self.pool_size)
        })
        #self.lora_down =nn.Parameter(torch.zeros((self.pool_size, in_features, r)),requires_grad=True)
        #self.lora_up = nn.Parameter(torch.zeros((self.pool_size, r, out_features)),requires_grad=True)
        ...
        self.Moe_TopK=Moe_TopK
        nn.init.zeros_(self.lora_route)
        for param in self.lora_down.values():
            nn.init.kaiming_uniform_(param, a=math.sqrt(5))
        for param in self.lora_up.values():
    ...
    def del_lora(self, input,task_id=-1):
        ...
        if task_id<=self.num_tasks:
            ...
            #lora_route_all=torch.cat([self.lora_route[0,:,0].unsqueeze(-1).detach(),self.lora_route[0,:,1:2],self.lora_route[0,:,2:].detach()],dim=1).unsqueeze(0)
            lora_route_all=torch.sum(self.lora_route,dim=0).unsqueeze(0)
        task_id = min(task_id, self.num_tasks+1)
        k=min(task_id-1,self.Moe_TopK-1) 
        input_mean=torch.mean(input,dim=1).unsqueeze(1)          
        lora_omegas=torch.mean(torch.matmul(input_mean,lora_route_all),dim=(0,1))
        lora_omegas_mean=lora_omegas[1:task_id]
        ...
        print(gate_value[-1])
        if gate_value[-1]<0.5:
            self.lora_up[str(task_id)]=0*self.lora_up[str(task_id)]
            self.lora_down[str(task_id)].requires_grad=False
            self.lora_up[str(task_id)].requires_grad=False
            delta_w_all=1
        return delta_w_all
        
        
    def loss_ortho(self, task_id=None,loss_type="FA",device=None):
        loss = loss_lora = loss_gate = loss_c = loss_w = 0
          # (task_id-1) x 8 x 8
        if  loss_type=="FAG":
            route_all = torch.cat([self.lora_route[:task_id-1].detach(), self.lora_route[task_id-1].unsqueeze(0)], dim=0).permute(0, 2, 1).reshape(-1, self.in_features)
            QQT_route = torch.mm(route_all, route_all.t())
            ...
            if task_id>1:
                replay_text_mean=torch.mean(self.replay_text,dim=1)
                loss_gate=torch.norm(torch.matmul(replay_text_mean.detach(),self.lora_route[task_id-1]),p='fro')
                loss=0.5*loss_w+loss_gate+loss_c
        return loss
    ...
    def forward(self, input,input2=None,task_id=-1):
        ...
        if self.training:
            ...
            for i, j in enumerate(I_list):
                if j == task_id:
                    lora_down_up = (self.lora_down[str(j)] @ self.lora_up[str(j)])
                else:
                    lora_down_up = (self.lora_down[str(j)] @ self.lora_up[str(j)]).detach()
                #omega_scaled_transformed = torch.mean(lora_omegas[:, :, j],dim=1).view(-1,1,1)* scaled_transformed
                transformed = self.selector(input @ lora_down_up)
                scaled_transformed = self.scale * self.dropout(transformed)
                omega_scaled_transformed = gate_value[i]* scaled_transformed 
...
x=torch.rand(2,77,768)
x2=torch.rand(1,77,768)
net=ContinualMoewithDel(768,320,num_tasks=5,pool_size=6,r=16,dropout_p=0,scale=1,Moe_TopK=3)
y=net(x,x2,task_id=4) 
class ContinualMultiLora(nn.Module):
    def __init__(
        self, in_features, out_features,bias=False,num_tasks=5,pool_size=10,r=8, dropout_p=0, scale=1
    ):
        super().__init__()
    ...
    def loss_ortho(self, task_id=None,loss_type="FA",device=None):
        ...
        if  loss_type=="FAG":
            ...
            loss_route=torch.norm(torch.mul(QQT_route,matrix_route), p='fro')
            loss=loss_route
            w0=self.linear.weight.unsqueeze(0).unsqueeze(0).repeat(self.delta_w.shape[0],self.delta_w.shape[1] , 1, 1)
            w=w0+self.delta_w.transpose(-1,-2)
            loss+=torch.norm(torch.einsum('ijkl,ijlm->ijkm', w,w.transpose(-1, -2))-torch.einsum('ijkl,ijlm->ijkm',w0,w0.transpose(-1, -2)),p='fro')
        elif loss_type=="FAO": 
                loss = torch.norm(torch.mul(QQT_lora,1-torch.eye(QQT_lora.shape[0],device=device)), p='fro')
        elif loss_type=="FA": 
                loss = torch.norm(QQT_lora - torch.eye(QQT_lora.shape[0], device=device), p='fro')    
    ...
    def forward(self, input,input2=None,task_id=-1):
        delta_w_all=0
        if task_id<=self.num_tasks:
            lora_route_all=torch.cat([self.lora_route[task_id-1,:,:2*task_id],self.lora_route[task_id-1,:,2*task_id:].detach()],dim=1).unsqueeze(0)
        else:
            #lora_route_all=torch.cat([self.lora_route[0,:,0].unsqueeze(-1).detach(),self.lora_route[0,:,1:2],self.lora_route[0,:,2:].detach()],dim=1).unsqueeze(0)
            lora_route_all=torch.sum(self.lora_route,dim=0).unsqueeze(0)
        task_id = min(task_id, self.num_tasks)
        if self.training:
            ...
            self.input_mean=input_mean
            lora_omegas=nn.functional.softmax(torch.matmul(input_mean,lora_route_all),2)
            lora_omegas_mean=torch.mean(lora_omegas,dim=(0,1))[:2*(task_id)]
            k=2
            G,I=torch.topk(lora_omegas_mean, 2, dim=-1)#3
            #tensor1 = torch.tensor([2 * (task_id - 1)]).to(I.device)
            #tensor2 = torch.tensor([2 * (task_id - 1) + 1]).to(I.device)
            #I=torch.cat((I,tensor1,tensor2),dim=0)
            I_list = I.tolist()
            print(I_list)
            #print(torch.mean(lora_omegas,dim=(0,1)))
            for j in I_list:
                if j == 2*(task_id-1) or j==2*(task_id-1)+1:
                    lora_down_up = (self.lora_down[j] @ self.lora_up[j])
                else:
                    lora_down_up = (self.lora_down[j] @ self.lora_up[j]).detach()
                scaled_transformed = self.scale * self.dropout(self.selector(input@lora_down_up))
...
x=torch.rand(2,77,768)
x2=torch.rand(1,77,768)
net=ContinualMultiLora(768,320,num_tasks=5,pool_size=10,r=8,dropout_p=0,scale=1)
y=net(x,x2,task_id=1) 
...
def inject_trainable_singlelora_continual(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        #print(name)
    return require_grad_params, names
def inject_trainable_loraEC_continual(
    model: nn.Module,
    target_replace_module: Set[str] = DEFAULT_TARGET_REPLACE,
    num_tasks=5,
    pool_size=5,
    ...
    for _module, name, _child_module in _find_modules(
        ...
        #print(name)
    return require_grad_params, names
def inject_trainable_loraEC2_continual(
    model: nn.Module,
    target_replace_module: Set[str] = DEFAULT_TARGET_REPLACE,
    num_tasks=5,
    pool_size=5,
